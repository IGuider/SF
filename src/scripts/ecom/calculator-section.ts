...
const animateNumericText = (
  ...
  const tick = (timestamp: number) => {
    ...
    const animatedValue =
      previousValue + (targetValue - previousValue) * easeInOutCubic(progress);
...
export const initCalculatorSections = () => {
  ...
  for (const calculator of document.querySelectorAll(".calculator-section")) {
    ...
    const trackShell = calculator.querySelector(
      ".calculator-section__track-shell",
    );
    const amountPanel = calculator.querySelector(
      ".calculator-section__panel--amount",
    );
    const plansPanel = calculator.querySelector(
      ".calculator-section__panel--plans",
    );
    ...
    const plansHeaderSlot = calculator.querySelector(
      "[data-calculator-plans-header-slot]",
    );
    ...
    const perksMobileSlot = calculator.querySelector(
      "[data-calculator-perks-mobile-slot]",
    );
    const perkInputs = [
      ...calculator.querySelectorAll(".calculator-section__perk-input"),
    ].filter(
    ...
    const baseMin = Number.parseInt(
      amountInput.dataset.min || rangeInput.min,
      10,
    );
    const baseMax = Number.parseInt(
      amountInput.dataset.max || rangeInput.max,
      10,
    );
    const maxDigits = Number.parseInt(
      amountInput.dataset.maxDigits || "12",
      10,
    );
    ...
    const paintRange = (value: number) => {
      ...
      const ratio =
        nextMax === nextMin ? 1 : (value - nextMin) / (nextMax - nextMin);
      const shellWidth = trackShell.clientWidth;
      const calculatorStyles = getComputedStyle(calculator);
      const isMobileViewport = window.matchMedia("(max-width: 768px)").matches;
      const thumbSizeVariable = isMobileViewport
        ? "--calculator-thumb-size-mobile"
        : "--calculator-thumb-size";
      const trackHeightVariable = isMobileViewport
        ? "--calculator-track-height-mobile"
        : "--calculator-track-height";
      const thumbSize =
        Number.parseFloat(
          calculatorStyles.getPropertyValue(thumbSizeVariable),
        ) || 0;
      const trackHeight =
        Number.parseFloat(
          calculatorStyles.getPropertyValue(trackHeightVariable),
        ) || 0;
    ...
    const updatePlans = (principal: number) => {
      ...
      for (const card of planCards) {
        const paymentPeriods = Number.parseInt(
          card.dataset.paymentPeriods || "0",
          10,
        );
        const firstPeriodRate = Number.parseFloat(
          card.dataset.firstPeriodRate || "0",
        );
        ...
        if (
          !(paymentNode instanceof HTMLElement) ||
          !(overpaymentNode instanceof HTMLElement) ||
          !(rateNode instanceof HTMLElement)
        ) {
          continue;
        }

        const { monthlyPayment, overpayment, overpaymentRate } =
          calculatePlanValues(principal, discountCount, {
            paymentPeriods,
            firstPeriodRate,
          });
        ...
        clearTimeout(
          card.dataset.updateTimeout
            ? Number.parseInt(card.dataset.updateTimeout, 10)
            : undefined,
        );
    ...
    amountInput.addEventListener("input", () => {
      const selectionStart =
        amountInput.selectionStart ?? amountInput.value.length;
      const digitsBeforeCaret = countDigits(
        amountInput.value.slice(0, selectionStart),
      );
    ...
    amountInput.addEventListener("blur", () => {
      const parsed = Number.parseInt(
        trimDigits(amountInput.value, maxDigits),
        10,
      );
      const normalizedValue = Number.isNaN(parsed)
        ? currentValue
        : Math.min(baseMax, Math.max(baseMin, parsed));
    ...
    amountInput.addEventListener("paste", (event) => {
      ...
      const caretDigitIndex = Math.min(
        countDigits(amountInput.value.slice(0, selectionStart)) +
          countDigits(pastedText),
        maxDigits,
      );
      const caretPosition = getCaretFromDigitIndex(
        formattedValue,
        caretDigitIndex,
      );
    ...
    cleanupTasks.push(() => {
      ...
      tabletPerksMedia.removeEventListener("change", syncTabletLayout);
      tabletPerksMedia.removeEventListener("change", syncPerksPlacement);
      compactCardsMedia.removeEventListener("change", syncCardDetailsLayout);
      tabletPerksMedia.removeEventListener("change", syncRangeAfterLayout);
      compactCardsMedia.removeEventListener("change", syncRangeAfterLayout);
      trackResizeObserver.disconnect();
      window.removeEventListener("load", handleWindowLoad);

      if (calculator instanceof HTMLElement) {
        delete calculator.dataset.calculatorBound;
        delete calculator.dataset.calculatorReady;
      }
    });
