...
const clientsEmblaMap = new WeakMap<HTMLElement, { embla: EmblaCarouselType; destroy: () => void }>();
let clientsMediaQueryBound = false;

const getWrappedIndex = (index: number, count: number) => ((index % count) + count) % count;
...
const getClientsNodes = (section: HTMLElement) => {
	...
	const paginationButtons = Array.from(section.querySelectorAll<HTMLButtonElement>('[data-clients-pagination] .clients-section__pagination-button'));
...
const resetClientsState = (section: HTMLElement) => {
	...
	const paginationButtons = section.querySelectorAll<HTMLButtonElement>('[data-clients-pagination] .clients-section__pagination-button');
