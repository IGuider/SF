...
const completeSectionAnimation = (section: HTMLElement, numbers: HTMLElement[]) => {
...
const animateSection = (section: HTMLElement) => {
	if (section.dataset.stepsAnimating === 'true' || section.dataset.stepsAnimated === 'true') {
		return;
	}
	...
	const checkpoints = numbers
		.map((number) => {
			...
			return {
				number,
				progress: Math.min(Math.max((center - timelineRect.left) / timelineRect.width, 0), 1),
			};
		})
	...
	const tick = (timestamp: number) => {
		...
		const rawProgress = Math.min((timestamp - startTime) / ANIMATION_DURATION_MS, 1);
		const progress = easeOutCubic(rawProgress);

		section.style.setProperty('--steps-timeline-progress', progress.toString());

		while (activatedCount < checkpoints.length && progress >= checkpoints[activatedCount].progress) {
	...
	for (const section of sections) {
		if (section.dataset.stepsBound === 'true') {
			if (section.dataset.stepsAnimated === 'true') {
				section.style.setProperty('--steps-timeline-progress', '1');
				activateAllNumbers([...section.querySelectorAll<HTMLElement>(NUMBER_SELECTOR)]);
			}
