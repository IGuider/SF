...
export const calculatePlanValues = (
  ...
  const monthlyPayment = roundMoney(
    calculateMonthlyPayment(principal, annualRate, config.paymentPeriods),
  );
  const firstPeriodCharge = roundMoney(
    principal * (config.firstPeriodRate / 100),
  );
  const overpayment = roundMoney(
    monthlyPayment * config.paymentPeriods + firstPeriodCharge - principal,
  );
...
export const getInitialPlanValues = (principal: number, discountCount = 0) =>
  CALCULATOR_PLANS.map((plan) =>
    calculatePlanValues(principal, discountCount, plan),
  );
