...
const tenderLoanFaq = defineCollection({
  loader: glob({ pattern: "**/*.json", base: "./src/content/tender-loan-faq" }),
  schema: z.object({
    question: z.string(),
    answer: z.string(),
    order: z.number().int().positive(),
...
const homeTrustItems = defineCollection({
  loader: glob({
    pattern: "**/*.json",
    base: "./src/content/home-trust-items",
  }),
  schema: z.object({
    title: z.string(),
    description: z.string(),
...
const socialLinks = defineCollection({
  ...
  schema: z.object({
    label: z.string(),
    href: z.string(),
    icon: z.enum(["vk", "telegram", "max", "mail"]),
    contexts: z
      .array(z.enum(["header", "footer", "blog"]))
      .default(["header", "footer", "blog"]),
    order: z.number().int().positive(),
  }),
});

const footerLinks = defineCollection({
  loader: glob({ pattern: "**/*.json", base: "./src/content/footer-links" }),
  schema: z.object({
    label: z.string(),
    href: z.string(),
    order: z.number().int().positive(),
...
const turnoverOptions = defineCollection({
  loader: glob({
    pattern: "**/*.json",
    base: "./src/content/turnover-options",
  }),
  schema: z.object({
    label: z.string(),
    order: z.number().int().positive(),
