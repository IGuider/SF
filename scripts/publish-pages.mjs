...
const remoteUrl = runOutput(gitCommand, ['config', '--get', 'remote.origin.url']);
const { owner, slug } = parseGitHubRemote(remoteUrl);
const userName =
  runOutput(gitCommand, ['config', '--get', 'user.name'], { stdio: 'pipe' }).trim() || owner;
const userEmail =
  runOutput(gitCommand, ['config', '--get', 'user.email'], { stdio: 'pipe' }).trim() ||
